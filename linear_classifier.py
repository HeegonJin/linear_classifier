...
def svm_loss_vectorized(W, X, y, reg):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  batch_num = X.shape[0] # N
  scores = X.mm(W) # (N, C)
  correct_class_score = scores[torch.arange(batch_num), y].reshape(batch_num, 1) # (N, 1)
  margin = torch.maximum(torch.zeros_like(scores), scores - correct_class_score + 1) #broadcast
  margin[torch.arange(batch_num), y] = 0

  loss = margin.sum() / batch_num
  loss += reg * torch.sum(W * W)
  ...
  # Replace "pass" statement with your code
  margin[margin > 0] = 1
  scores = margin.sum(dim=1)
  margin[torch.arange(batch_num), y] -= scores
  dW = (X.T).mm(margin)
  dW /= batch_num
  dW += reg * 2* W
  #############################################################################
...
def sample_batch(X, y, num_train, batch_size):
  ...
  #########################################################################
  # Replace "pass" statement with your code
  rand_num = torch.randint(0, num_train, (batch_size, ))
  X_batch = X[rand_num]
  y_batch = y[rand_num]
  #########################################################################
  #                       END OF YOUR CODE                                #
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [7e-3, 1e-3, 5e-3, 9e-3, 1e-2]
  regularization_strengths = [1e-3, 5e-3, 9e-4, 5e-4, 7e-4]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  # Softmax Loss
  for i in range(num_train):
    f = scores[i] - torch.max(scores[i]) # avoid numerical instability
    softmax = torch.exp(f)/torch.sum(torch.exp(f))
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  num_train = X.shape[0]
  num_classes = W.shape[1]
  f = scores - (torch.max(scores, dim=1, keepdim=True)[0])
  softmax = torch.exp(f) / torch.sum(torch.exp(f), dim=1, keepdim=True)
  loss += torch.sum(-torch.log(softmax[torch.arange(num_train), y]))

  softmax[np.arange(num_train),y] -= 1 # cheated
  dW = X.T.mm(softmax)
